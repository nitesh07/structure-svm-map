...
#define PYTHON_WRITE_LABEL_BYNAME		"write_label_byname"
...
PyObject* smToPythonObject(STRUCTMODEL *sm)
{
    ...
    DEBUG("Using TUPLE, and sizePsi = %d,array_size[0]=%d,pStartingIndex=%d\n",sm->sizePsi,array_size[0],pStartingIndex);
    array = PyTuple_New(array_size[0]);
    if (sm->sizePsi > 0) {
        for (i=1-pStartingIndex; i<=sm->sizePsi; ++i) {
            PyTuple_SET_ITEM(array, i-1+pStartingIndex, PyFloat_FromDouble(sm->w ? sm->w[i] : 0.0));
        }
    }
#endif
                                    ...
                                    STRUCT_LEARN_PARM *sparm)
{
    /* Initializes the optimization problem. Typically, you do not need
       ...
       set of constraints. */
    DEBUG("in init_struct_constraints\n");
    CONSTSET c;
                                    ...
                                    STRUCT_LEARN_PARM *sparm)
{
    ...
    DEBUG("In classify_struct_example\n");
    LABEL y;
...
STRUCTMODEL read_struct_model(char *file, STRUCT_LEARN_PARM *sparm)
{
    ...
    //DEBUG("W= ",sm.w[i+1-pStartingIndex]);
    for (i=0+pStartingIndex; i<n+pStartingIndex; ++i) {
        ...
        //DEBUG("%.4f ",sm.w[i+1-pStartingIndex]);
        Py_DECREF(pPsiValue);
    }
    //DEBUG("\n");
    Py_DECREF(pValue);
...
void        write_label(FILE *fp, LABEL y)
{
    ...
    if (pFunc == NULL) {
        /*fprintf(stderr, "Warning: could not find function %s!\n",
          PYTHON_WRITE_LABEL);*/
        DEBUG("Perform default behavior\n");
...
{
    DEBUG("IN write_label_ByName\n");
    /* Writes label y to file handle fp. */
    PyObject *pDict, *pFunc, *pArgs, *pValue;
    /* Set up the call to the Python method. */
    pDict = PyModule_GetDict(pModule);
    pFunc = PyDict_GetItemString(pDict, PYTHON_WRITE_LABEL_BYNAME);
    if (pFunc == NULL) {
        fprintf(stderr, "Warning: could not find function %s!\n", PYTHON_WRITE_LABEL_BYNAME);
        return;
    }
    /* Build the argument list. */
    Py_INCREF((PyObject*)y.py_label);
